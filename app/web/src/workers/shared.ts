...
export const DockerStateChannelName = 'docker-state'

const ctx: SharedWorkerGlobalScope = self as any // eslint-disable-line @typescript-eslint/no-explicit-any

// execute only in shared worker context
if (typeof ctx === 'object' && ctx.constructor.name.toLowerCase().includes('worker')) {
  // kinda fuse
  const db = new DB()
  const api = new API()
