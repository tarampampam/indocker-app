...
us to understand how many users use the daemon.

The data is sent to the server via HTTPS and is not stored anywhere except the statistics server, and is not used for
any other purposes.

You can disable the statistics collecting by passing the `--dont-send-anonymous-usage` flag to the daemon, but we will
...
| `--dont-send-anonymous-usage` | Don't send anonymous usage statistics (please, leave it enabled, it helps us to improve the project) |            `false`            | `DOCKER_WATCH_INTERVAL` |
