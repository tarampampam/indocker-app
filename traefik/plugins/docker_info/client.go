...
func NewClient(unixSocket string) *Client {
  return &Client{unixSocket: unixSocket}
}

// init initializes the client (establishes a connection to the Docker socket).
func (c *Client) init(ctx context.Context) (err error) {
  c.connect.Do(func() {
    c.conn, err = (&net.Dialer{}).DialContext(ctx, "unix", c.unixSocket)

    if err == nil {
      c.http = &http.Client{
        Transport: &http.Transport{
          DialContext: func(_ context.Context, _, _ string) (net.Conn, error) { return c.conn, nil },
        },
      }
    }
  })
